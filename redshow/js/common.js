...
function login_popup() {
	$("#id_account_l").val(""), $("#id_password_l").val(""), $("#id_email").val(""), $("#id_password").val(""), $("#id_captcha_1").val(""), $("#id_mobile").val(""), $("#id_mobile_code").val(""), $("#id_password_m").val(""), $("#id_captcha_m_1").val(""), $("#id_captcha_1").val(""), $("#login-form-tips").hide(), $("#findpassword-tips").hide(), $("#mobile_code_password-tips").hide(), $("#register-tips").hide(), $("#loginModal").modal("show")
}
...
function login_form_submit() {
	$.ajax({
		...
		success: function(e) {
			if(e.account_l) $("#login-form-tips").html(e.account_l).show(500), $("#id_account_l").focus();
			else if(e.password_l) $("#login-form-tips").html(e.password_l).show(500), $("#id_password_l").focus();
			else {
				if("success" == e.status) return "True" == e.onepay_status ? window.location.replace("/") : window.location.replace(e.url), detect_ie() !== !1 && window.location.reload(), void 0;
				"failure" == e.status && ("no_active" == e.msg ? (zyemail = $("#id_account_l").val(), zyUname = zyemail, zy_validate_Email(!1, zyemail, $("#id_password_l").val())) : $("#login-form-tips").html("账号或者密码错误，请重新输入").show(500))
			}
		},
		complete: function() {
...
function show_send_sms(e) {
	return $("#forgetpswMobileModal .send-verify,#verify_form .send-verify").html("重发验证码（" + e + "s）").css({
		background: "#D6D6D6"
	}).attr("disabled", "disabled"), 0 >= e ? ($(".verify-tips.success").hide(500), $("#forgetpswMobileModal .send-verify,#verify_form .send-verify").html("重发验证码").css({
		background: "#5ECFBA"
	}).removeAttr("disabled"), sendOFF = !0, void 0) : (e--, setTimeout("show_send_sms(" + e + ")", 1e3), void 0)
}

function send_sms_code() {
	show_send_sms(60);
	var e = $("#id_mobile_code").val(),
		s = "";
	("" == e || void 0 == e) && (e = $("#id_mobile_f").val(), s = "change"), sendOFF && ($.ajax({
		...
		success: function(e) {
			e.mobile ? "" != s ? $("#mobile_code_password_form_message").css({
				background: "#f2dede"
			}).html(e.mobile) : $(".verify-tips").removeClass("success").addClass("error").html(e.mobile).show(500) : e.captcha ? "" != s ? $("#mobile_code_password_form_message").css({
				background: "#f2dede"
			}).html(e.captcha) : $(".verify-tips").removeClass("success").addClass("error").html(e.captcha).show(500) : "success" == e.status ? "" != s ? $("#mobile_code_password_form_message").css({
				background: "#f2dede"
			}).html("") : ($(".verify-tips").removeClass("error").addClass("success").html(e.info).show(500), show_send_sms(60)) : "failure" == e.status && ("" != s ? $("#mobile_code_password_form_message").css({
				background: "#f2dede"
			}).html("请稍后再试") : $(".verify-tips").removeClass("success").addClass("error").html("短信验证码发送失败").show(500))
		},
		complete: function() {
...
function Search() {
	...
	$(".search-btn").click(function() {
		e($("#data-search").val()), zhuge.track("搜索次数", {
			"搜索关键词": $("#data-search").val()
		})
...
function yueKeLastStep(e) {
	var s = $("#yueke-last-step"),
		o = $("#ops-user-name"),
		a = $("#ops-phone-num"),
		t = $("#ops-advisor"),
		i = $("#ops-verification"),
		c = /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
	i.val(), s.modal({
		show: !0,
		keyboard: !1,
		backdrop: "static"
...
$(".class53").click(function() {
	...
	}), $(".toolbar-item-weibo>div").hover(function() {}, function() {
		$(this).stop().hide("fast")
...
$(".finished").parent().on("click", function() {
	var e = $(this).attr("meeting_id"),
		s = $(this).attr("href_url");
	$.ajax({
		type: "POST",
